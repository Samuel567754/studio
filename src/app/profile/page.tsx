    ...
    <div className="space-y-8 max-w-3xl mx-auto">
      ...
      <Card className="shadow-lg border-primary/20">
        ...
        <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-6 text-lg">
          <div className="flex items-center space-x-3 p-4 bg-secondary/30 rounded-lg shadow-sm">
                ...
                {profileData.practiceWords.length > 0 ? (
                    <ScrollArea className="h-48 w-full rounded-md border p-3 bg-background/50">
                        ...
                        {profileData.practiceWords.map((word, index) => (
                            <Badge key={`practice-${index}`} variant="secondary" className="text-sm px-2 py-1">{word}</Badge>
                        ))}
                        </div>
                    </ScrollArea>
                ) : (
                 ...
                 {profileData.masteredWords.length > 0 ? (
                    <ScrollArea className="h-48 w-full rounded-md border p-3 bg-background/50">
                        ...
                        {profileData.masteredWords.map((word, index) => (
                            <Badge key={`mastered-${index}`} variant="success" className="text-sm px-2 py-1 bg-green-500/10 text-green-700 dark:bg-green-500/20 dark:text-green-400 border-green-500/30">{word}</Badge>
                        ))}
                        </div>
                    </ScrollArea>
                ) : (
