...
import { CheckCircle2, XCircle, Target, Lightbulb, Smile } from 'lucide-react';
...
const generateMcqOptions = (correctWord: string, wordPool: string[], numOptions: number = 4): string[] => {
  ...
  if (options.length < 2 && wordPool.length > 1) { 
  ...
  return options.length > 0 ? options : [correctWord]; 
};
...
export const WordIdentificationGame: FC<WordIdentificationGameProps> = ({
  ...
  const handleOptionClick = (option: string) => {
    ...
    onGameResult(correct, option); // This will trigger the toast in the parent component
          ...
          {options.map((option, index) => (
            <Button
              ...
              className={cn(
                ...
                isAttempted && option !== selectedOption && option.toLowerCase() === wordToIdentify.toLowerCase() && "bg-green-500/10 border-green-500/50 text-green-600 dark:text-green-500", 
      ...
      <CardFooter className="flex flex-col gap-4 pt-4 border-t border-border/20">
        {isAttempted && isCorrect !== null && (
          <Alert variant={isCorrect ? "success" : "destructive"} className="w-full animate-in fade-in-0 zoom-in-95 duration-300">
            ...
            <AlertTitle>
              {isCorrect ? 
                <div className="flex items-center gap-1"><Smile className="h-5 w-5" /> {username ? `Spot on, ${username}!` : 'Correct!'}</div> : 
                'Not Quite!'}
            </AlertTitle>
            <AlertDescription>
