...
export function EnglishTutorAssistant() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const input: EnglishTutorInput = { question: userMessage.text, username: username || undefined };
      const result = await englishTutor(input);
      ...
      console.error("Error with English Tutor AI:", error);
      const errorMessage: ChatMessage = {
        id: Date.now().toString() + '-error',
        type: 'ai',
        text: "Sorry, I'm having a little trouble understanding right now. Please try again in a moment.",
      };
      ...
      <CardContent className="space-y-4">
        <ScrollArea ref={scrollAreaRef} className="h-80 w-full rounded-md border p-4 bg-background/60 shadow-inner">
          {chatHistory.length === 0 && (
             <div className="flex flex-col items-center justify-center h-full text-center text-muted-foreground">
                ...
                <p className="text-sm">No messages yet. What would you like to ask about English?</p>
             </div>
          )}
              ...
              <div className="flex flex-col gap-1">
                ...
                {message.type === 'ai' && soundEffectsEnabled && (
                  <Button 
                    ...
                    aria-label="Read Lexi's message aloud"
                  >
        ...
        <form onSubmit={handleSubmit} className="flex items-center gap-2 pt-2 border-t border-border/30">
          <Input
            ...
            disabled={isLoading}
            aria-label="Your question for Lexi"
          />
